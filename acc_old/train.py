...
def train(args):
    ...
    # total_params = sum(p.numel() for p in model.net_coarse.parameters() if p.requires_grad)
    ...
    while global_step < model.start_step + args.n_iters + 1:
        ...
        for train_data in train_loader:
            ...
            # print(train_data["rgb_path"], torch.max(ray_batch["src_rgbs"]), torch.min(ray_batch["src_rgbs"]), \
            #       torch.mean(ray_batch["src_rgbs"]), torch.var(ray_batch["src_rgbs"]))
            ...
            # Rest is logging
            if args.local_rank == 0:
                if global_step % args.i_print == 0 or global_step < 720100:
                    # write mse and psnr stats
                    # print(ret["outputs_coarse"]["rgb"], ray_batch["rgb"])
                    ...
                    # logstr += " path: {}".format(train_data["rgb_path"])
                    print(logstr)
                    print("each iter time {:.05f} seconds".format(dt))
                if np.isnan(loss.item()):
                    print("Network loss is NaN!")
                    print(train_data["rgb_path"])
                    break
...
def log_view(
    ...
    imageio.imwrite(filename, (rgb_im*255.0).astype(np.uint8))
    if depth_im is not None:
        ...
        imageio.imwrite(filename, (depth_im*255.0).astype(np.uint8))
