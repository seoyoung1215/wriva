...
def eval(args):
    ...
    # total_params = sum(p.numel() for p in dir(model) if hasattr(getattr(model, p), 'requires_grad'))
    # print(total_params)
    # create projector
...
def log_view(
    ...
    rgb_gt = rgb_gt.permute(1, 2, 0).detach().cpu().numpy()
    rgb_gt = np.clip(rgb_gt, 0.0, 1.0)
    filename = os.path.join(out_folder, prefix[:-1] + "_{:03d}_gt.png".format(global_step))
    imageio.imwrite(filename, (rgb_gt*255.0).astype(np.uint8))
    ...
    rgb_coarse = np.clip(rgb_coarse, 0.0, 1.0)
    # print("output rgb", np.unique(rgb_coarse))
    filename = os.path.join(out_folder, prefix[:-1] + "_{:03d}_coarse.png".format(global_step))
    # import pdb; pdb.set_trace()
    imageio.imwrite(filename, (rgb_coarse*255.0).astype(np.uint8))
